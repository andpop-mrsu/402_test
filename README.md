# Лабораторные работы для курса "Технологии создания приложений баз данных"
## Мордовский госуниверситет, ФМиИТ, 402 группа

### Блоки заданий
1. **Встраиваемая реляционная СУБД, консольное приложение**. Написать консольную игру на языке PHP, работающую с встраиваемой БД SQLite. Описание задачи в файле db_specification_01_nn.md (nn - номер варианта).
2. **NoSQL СУБД, веб-приложение**. Написать веб-приложение (JavaScript), работающее с IndexedDB.
3. **Реляционная СУБД, веб-приложение**. Написать веб-приложение, использующее ORM и работающее с СУБД MySQL.

* * *

* Задания для выполнения размещаются в файлах db_task01_nn.md, db_task02_nn.md, ..., где nn - номер варианта.
* Проекты для решения заданий размещаются в папках DB_Task01, DB_Task02, DB_Task03, ...
* Прав на запись (коммиты) в данный репозиторий у студентов нет, файлы с решениями должны передаваться из личных репозиториев через механизм Pull Requests.
* Задание будет считаться принятым после закрытия соответствующего Pull Request в этом репозитории.

